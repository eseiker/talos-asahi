...
KERNEL_FLAVOR=upstream "${root}/scripts/prepare-sources.sh" "${validation_root}/upstream"
...
if ! git -C "${validation_root}/upstream/pkgs" diff --quiet; then
  printf 'upstream kernel flavor must use the unmodified pinned pkgs source\n' >&2
  exit 1
fi
...
printf 'Asahi, mainline 16K, mainline 4K, and upstream sources, lifecycle, and sd-boot validation passed for %s\n' \
  "${RELEASE_TAG}"
